...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('source', sa.Enum('POLL', 'WEBSOCKET', 'BACKFILL', name='datasource'), nullable=False),
    ...
    op.create_table(
        'backtest_results',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('strategy', sa.Enum('LONG_FAVORITE', 'FADE_OVERREACTION', 'MEAN_REVERSION', 'MOMENTUM', name='strategytype'), nullable=False),
    ...
    op.create_index(op.f('ix_backtest_executions_backtest_id'), 'backtest_executions', ['backtest_id'])
